...
protected:
    ...
    // If bGetup is set to true then the agent will stand up in case of a fall, otherwise
    // it won't stand up.
    bool bGetup;
    bool bAutoGetup;
    // AI lab Hoseini 95-10-07 end
...
public:
    ...
    //AI lab hoseini 95-06-11 start
    inline BodyModel* getBodyModel()
    {
        return bodyModel;
    }
    inline WorldModel* getWorldModel()
    {
        return worldModel;
    }
    //AI lab hoseini 95-06-11 end

    //AI lab hoseini 95-09-24 start
    map< SkillType, boost::shared_ptr<Skill> > getSkills()
    {
        return skills;
    }
    void addKeyframeToSkill(SkillType skillIndex, boost::shared_ptr<KeyFrame> keyframe);

    void deleteKeyframeFromSkill(SkillType skillIndex, int keyframeIndex);
    void deleteAllKeyframesFromSkill(SkillType skillIndex);

    void editSkill(SkillType skillIndex, int &keyframeIndex, boost::shared_ptr<KeyFrame> &newKeyframeData);

    void resetSkill(SkillType skillIndex);

    inline void setKeyframeToExecuteIndex(SkillType skillType, int keyframeIndex)
    {
        skills[ skillType ]->setKeyframeToExecuteIndex(keyframeIndex);
    }
    inline void setBGetup(bool b)
    {
        bGetup = b;
    }
    inline void setBAutoGetup(bool b)
    {
        bAutoGetup = b;
    }
