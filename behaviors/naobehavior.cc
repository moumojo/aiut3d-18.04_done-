    ...
    namedParams( namedParams_ ), rsg( rsg_ )
{
    ...
    // Tell the agent not to get up automatically after a fall
    bGetup = false;
    bAutoGetup = false;
    ...
    try {
        ...
        // Load an empty skill, just to avoid core dump while executing SKILL_NONE
        readSkillsFromFile( "./skills/none.skl" );


        skills[SKILL_MOTION_EDITOR] = boost::shared_ptr<Skill>(new Skill());
...
//    worldModel->getRVSender()->drawCircle(ballPos.getX(), ballPos.getY(), 0.1, 1,1,1);

    // draw text on ball
//    stringstream ballRange;
//    ballRange << ballPos.getX() << " (m)";
//    worldModel->getRVSender()->drawText(ballRange.str(), ballPos.getX(), ballPos.getY(), 1,1,1);
...
void NaoBehavior::act()
{
    ...
    if((bAutoGetup || bGetup) && checkingFall())
    {
        resetSkills();
    ...
    {
        // AI lab Hoseini 95-10-07 start
        // reset bGetup as well
        bGetup = false;
        // AI lab Hoseini 95-10-07 start
    ...
    else
    {
        ...
        // AI lab Hoseini 95-12-2 start
        // The following if condition is modified so that after executing the last keyframe of the motion editor
        // skill, the agent state won't reset and it will keep the joint configuration dictated by the last
        // keyframe of the motion editor skill
        // AI lab Hoseini 95-12-2 end
        if((skills[skill]->done( bodyModel, worldModel) && skill != SKILL_MOTION_EDITOR) || bodyModel->useOmniWalk())
        {
            skills[skill]->reset();
            resetScales();
...
//    LOG_ST(skill);
    ...
    /// In this section we temporarily set the PID coefficient k1
    /// based on the current keyframe index
...
void NaoBehavior::readSkillsFromFile( const std::string& filename)
{
    ...
    parse_info<iterator_t> info = parse( skillDescription.c_str(), parser, ( space_p | comment_p("#") ) );
...
void NaoBehavior::addKeyframeToSkill(SkillType skillIndex, boost::shared_ptr<KeyFrame> keyframe)
{
    skills[skillIndex]->appendKeyFrame(keyframe);
}


void NaoBehavior::deleteKeyframeFromSkill(SkillType skillIndex, int keyframeIndex)
{
    skills[ skillIndex ]->deleteKeyframe(keyframeIndex);    
}

void NaoBehavior::deleteAllKeyframesFromSkill(SkillType skillIndex)
{
    skills[ skillIndex ]->deleteAllKeyframes();
}

void NaoBehavior::editSkill(SkillType skillIndex, int &keyframeIndex, boost::shared_ptr<KeyFrame> &newKeyframeData)
{
    skills[skillIndex]->editKeyframe(keyframeIndex, newKeyframeData);
}

void NaoBehavior::resetSkill(SkillType skillIndex)
{
